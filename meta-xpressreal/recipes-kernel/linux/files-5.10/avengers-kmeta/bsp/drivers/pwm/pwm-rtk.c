...
static void pwm_set_register(struct rtk_pwm_chip *pc, int hwpwm)
{
	if (pc == NULL) {
		...
		pr_debug("%s: PWM %d can't be set because it is isolated\n",
			 __func__, hwpwm);
		return;
	}
...
static int rtk_pwm_probe(struct platform_device *pdev)
{
	...
	if (pwm->pwm_reg_set == pwm_reg_set_v2)
		max_csd = RTK_PWM_V2_CSD_MASK;
	else if (pwm->pwm_reg_set == pwm_reg_set_v3)
		max_csd = RTK_PWM_V3_CSD_MASK;
	else
		max_csd = RTK_PWM_V1_CSD_MASK;
