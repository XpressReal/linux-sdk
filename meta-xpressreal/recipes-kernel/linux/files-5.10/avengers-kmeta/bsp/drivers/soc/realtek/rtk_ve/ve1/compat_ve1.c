...
long rtk_ve1_compat_vpu_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	long ret = 0;
	...
	case COMPAT_VDI_IOCTL_GET_INSTANCE_POOL:
	{
		...
		instance_pool = rtk_ve1_get_instance_pool();
	...
	case COMPAT_VDI_IOCTL_GET_COMMON_MEMORY:
	{
		...
		/* s_common_memory is shared state; serialize like the other paths */
		ret = rtk_ve1_down_interruptible();
		if (ret != 0)
			return ret;

		common_memory = rtk_ve1_get_common_memory();
		...
		if (common_memory->base != 0) {
			...
			if (rtk_ve1_alloc_dma_buffer(common_memory) != 0) {
				rtk_ve1_sem_up();
				return -ENOMEM;
			}
	...
	case COMPAT_VDI_IOCTL_OPEN_INSTANCE:
	{
		...
		if (ret) {
			 ...
			 * sees the success, so close_inst undoes the list add
			 * and the s_vpu_open_ref_count bump. */
	...
	case COMPAT_VDI_IOCTL_GET_REGISTER_INFO: {
		...
		vpu_register = rtk_ve1_get_vpu_register();
	...
	{
		compat_vpudrv_buffer_t data32;
		vpudrv_buffer_t *bond_register;

		if (g_chip_type != CHIP_TYPE_RTD16XXB)
			return -ENOIOCTLCMD;

		bond_register = rtk_ve1_get_bond_register();

		data32 = (compat_vpudrv_buffer_t) {
			.size      = bond_register->size,
			.phys_addr = bond_register->phys_addr,
			.base      = bond_register->base,
			.virt_addr = bond_register->virt_addr,
			.mem_type  = bond_register->mem_type,
		};
	...
	case COMPAT_VDI_IOCTL_GET_RTK_DCSYS_INFO:
	{
		...
		dc_register = rtk_ve1_get_dc_register();
		dmc_register = rtk_ve1_get_dmc_register();
...
EXPORT_SYMBOL(rtk_ve1_compat_vpu_ioctl);
