...
do_toggle:
	...
	if (phy_cfg->force_setting) {
		u8 addr = PAGE0_0XF3;
		u8 data;

		rtk_phy_set_page(phy_reg, 0);
		data = rtk_phy_read(phy_reg, PAGE0_0XF3);
		if (data != FORCE_SETTING)
			rtk_phy_write(phy_reg, addr, FORCE_SETTING);
		mdelay(1);
		rtk_phy_write(phy_reg, addr, data);
		if (phy_cfg->do_toggle_once)
			phy_cfg->force_setting = false;
	}
#endif
...
static int rtk_phy_init(struct phy *phy)
{
	...
	for (i = 0; i < rtk_phy->num_phy; i++) {
		if (rtk_phy->phy_dis_map & (0x1 << i))
			continue;

		ret = do_rtk_phy_init(rtk_phy, i);
		if (ret) {
			dev_err(rtk_phy->dev, "%s phy#%d init fail (ret=%d)\n",
				__func__, i, ret);
		}
	}
...
static void rtk_phy_toggle(struct rtk_phy *rtk_phy, bool connect, int port)
{
	...
	if (rtk_phy->phy_dis_map & (0x1 << index))
		return;
...
static int parse_phy_data(struct rtk_phy *rtk_phy, const struct phy_cfg *phy_cfg)
{
	...
	rtk_phy->phy_dis_map = 0;

	if (rtk_phy->num_phy > 1) {
		struct soc_device_attribute rtk_soc_kent_big_pkg[] = {
			{ .family = "Realtek Kent", .soc_id = "RTD1861B" },
			{ .family = "Realtek Kent", .soc_id = "RTD1501B" },
			{ /* empty */ } };

		if (soc_device_match(rtk_soc_kent_big_pkg)) {
			/* disable usb1 phy0 and phy2 */
			rtk_phy->phy_dis_map |= BIT(0);
			rtk_phy->phy_dis_map |= BIT(2);
			dev_info(dev, "%s disable port3 and port4 phy disable map: 0x%x\n",
				 __func__, rtk_phy->phy_dis_map);
		}
		dev_dbg(dev, "%s phy disable map: 0x%x\n", __func__, rtk_phy->phy_dis_map);
	}
	...
	rtk_phy->phy_cfg = devm_kzalloc(dev, sizeof(struct phy_cfg) * rtk_phy->num_phy, GFP_KERNEL);
	for (i = 0; i < rtk_phy->num_phy; ++i)
		memcpy(&rtk_phy->phy_cfg[i], phy_cfg, sizeof(struct phy_cfg));
