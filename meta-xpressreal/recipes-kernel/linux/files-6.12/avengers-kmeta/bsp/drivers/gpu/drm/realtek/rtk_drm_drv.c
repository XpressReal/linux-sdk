...
static void rtk_drm_debugfs_init(struct drm_minor *minor)
{
...
#ifdef CONFIG_CHROME_PLATFORMS
	DRM_IOCTL_DEF_DRV(RTK_GEM_CREATE, rtk_gem_create_ioctl, DRM_AUTH | DRM_RENDER_ALLOW),
	DRM_IOCTL_DEF_DRV(RTK_GEM_MAP_OFFSET, rtk_gem_map_offset_ioctl, DRM_AUTH | DRM_RENDER_ALLOW),
#endif
...
static void rtk_drm_remove(struct platform_device *pdev)
{
...
static int __init rtk_drm_init(void)
{
	...
	ADD_RTK_SUB_DRIVER(rtk_prince_dptx_driver, CONFIG_DRM_RTK);
