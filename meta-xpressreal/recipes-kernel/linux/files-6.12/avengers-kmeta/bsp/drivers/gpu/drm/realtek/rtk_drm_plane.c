...
static ssize_t enable_osd_display_store(struct device *dev,
...
static DEVICE_ATTR_WO(enable_osd_display);

static ssize_t enable_sub_display_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct drm_device *drm = dev_get_drvdata(dev);
	struct rtk_drm_private *priv = drm->dev_private;
...
static DEVICE_ATTR_WO(enable_sub_display);

static ssize_t enable_video_display_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct drm_device *drm = dev_get_drvdata(dev);
	struct rtk_drm_private *priv = drm->dev_private;
...
static DEVICE_ATTR_WO(enable_video_display);
									...
									struct dma_buf_attachment *attach)
{
	...
	void *vaddr = rtk_plane->refclock_base;
...
static void rtkplane_release(struct dma_buf *dmabuf)
{
	...
	if (!rtk_plane || !rtk_plane->plane.dev) {
		pr_warn("rtkplane_release: NULL plane or device\n");
		return;
	}

	drm = rtk_plane->plane.dev;
	dev = drm->dev;
	daddr = rtk_plane->refclock_dma_handle;
	vaddr = rtk_plane->refclock_base;
	size = dmabuf->size;
...
static int write_cmd_to_ringbuffer(struct rtk_drm_plane *rtk_plane, void *cmd)
{
	...
	struct tag_ringbuffer_header_ptrs *rb_ptrs = &rtk_plane->ringheader_ptrs;
	...
	read = ipcReadULONG((u8 *)rb_ptrs->p_read_ptr, type);
	write = ipcReadULONG((u8 *)rb_ptrs->p_write_ptr, type);
	base = ipcReadULONG((u8 *)rb_ptrs->p_begin_addr, type);
	b_size = ipcReadULONG((u8 *)rb_ptrs->p_size, type);
	limit = base + b_size;
	...
	if (read + (read > write ? 0 : limit - base) - write > size) {
		...
		*rb_ptrs->p_write_ptr = ipcReadULONG((u8 *)&write, type);
...
static int rtk_plane_inband_config_disp_win(struct drm_plane *plane, struct rpc_config_disp_win *disp_win)
{
	...
	inband_cmd = kzalloc(sizeof(*inband_cmd), GFP_KERNEL);
	if (!inband_cmd) {
		DRM_ERROR("%s malloc inband_cmd fail\n", __func__);
		return -1;
	}
...
static int rtk_plane_update_cursor(struct drm_plane *plane)
{
	...
	unsigned int flags __maybe_unused = 0;
	int i;

	struct graphic_object *obj = kzalloc(sizeof(*obj), GFP_KERNEL);
...
static int rtk_plane_update_video_obj(struct drm_plane *plane)
{
	...
	int i;
	int index;
	struct video_object *obj = kzalloc(sizeof(*obj), GFP_KERNEL);

	if (!obj) {
		DRM_ERROR("%s malloc video_object fail\n", __func__);
		return -1;
	}
...
bool rtk_plane_check_update_done(struct rtk_drm_plane *rtk_plane)
{
	...
	if (rtk_plane->refclock_base) {

		if (rpc_info->krpc_vo_opt != RPC_AUDIO)
			refContext = *rtk_plane->refclock_ptrs.p_video_context;
		else
			refContext = htonl(*rtk_plane->refclock_ptrs.p_video_context);

		if (refContext == rtk_plane->context)
			return true;
	}

	trace_rtk_drm_context_update_fail(
		(rtk_plane->layer_nr == VO_VIDEO_PLANE_OSD1) ? TRACE_OSD1_PLANE :
		(rtk_plane->layer_nr == VO_VIDEO_PLANE_SUB1) ? TRACE_SUB1_PLANE :
		(rtk_plane->layer_nr == VO_VIDEO_PLANE_OSD2) ? TRACE_OSD3_PLANE :
		TRACE_OSD4_PLANE, rtk_plane->context);
...
static int rtk_plane_get_cache_config(struct rtk_rpc_info *rpc_info, int *cacheable)
{
	...
	ret = rpc_query_out_fw_memory_config(rpc_info, &rpc, &result);
	if (!ret)
		*cacheable = result.private_info[0];

	pr_info("%s *cacheable = %d\n", __func__, *cacheable);
			      ...
			      enum VO_VIDEO_PLANE layer_nr)
{
	...
	unsigned int id;
	bool is_cacheable = rtk_plane->fw_mem_cfg;
	...
	DRM_INFO("[%s] is_cacheable = %d\n", __func__, is_cacheable);

	videoplane = layer_nr;
	rpc_info->is_cacheable = is_cacheable;
	if (rpc_info->krpc_vo_opt != RPC_AUDIO)
	...
	rtk_drm_tag_ringbuffer_header_ptrs_init(
		&rtk_plane->ringheader_ptrs, vaddr + (64*1024), is_cacheable);
	...
	rtk_plane->refclock_base = vaddr;
	rtk_drm_tag_refclock_ptrs_init(&rtk_plane->refclock_ptrs, vaddr, is_cacheable);
	...
	*rtk_plane->refclock_ptrs.p_rcd = htonll(-1LL);
	*rtk_plane->refclock_ptrs.p_rcd_ext = htonl(-1L);
	*rtk_plane->refclock_ptrs.p_master_gpts = htonll(-1LL);
	*rtk_plane->refclock_ptrs.p_gpts_timeout = htonll(0LL);
	*rtk_plane->refclock_ptrs.p_video_system_pts = htonll(-1LL);
	*rtk_plane->refclock_ptrs.p_audio_system_pts = htonll(-1LL);
	*rtk_plane->refclock_ptrs.p_video_rpts = htonll(-1LL);
	*rtk_plane->refclock_ptrs.p_audio_rpts = htonll(-1LL);
	*rtk_plane->refclock_ptrs.p_video_context = htonl(-1);
	*rtk_plane->refclock_ptrs.p_audio_context = htonl(-1);
	*rtk_plane->refclock_ptrs.p_video_end_of_segment = htonl(-1);
	*rtk_plane->refclock_ptrs.p_video_free_run_threshold = htonl(0x7FFFFFFF);
	*rtk_plane->refclock_ptrs.p_audio_free_run_threshold = htonl(0x7FFFFFFF);
	*rtk_plane->refclock_ptrs.p_vo_underflow = htonl(0);
	*rtk_plane->refclock_ptrs.p_ao_underflow = htonl(0);
	rtk_plane->refclock_ptrs.p_mastership->systemMode = (unsigned char)AVSYNC_FORCED_SLAVE;
	rtk_plane->refclock_ptrs.p_mastership->videoMode = (unsigned char)AVSYNC_FORCED_MASTER;
	rtk_plane->refclock_ptrs.p_mastership->audioMode = (unsigned char)AVSYNC_FORCED_MASTER;
	rtk_plane->refclock_ptrs.p_mastership->masterState = (unsigned char)AUTOMASTER_NOT_MASTER;
	...
	if (rpc_info->krpc_vo_opt != RPC_AUDIO) {
		*rtk_plane->ringheader_ptrs.p_begin_addr = (long)(0xffffffff&(rtk_plane->dma_handle));
		*rtk_plane->ringheader_ptrs.p_size = 64*1024;
		*rtk_plane->ringheader_ptrs.p_buffer_id = 1;
	} else {
		*rtk_plane->ringheader_ptrs.p_begin_addr = htonl((long)(0xffffffff&(rtk_plane->dma_handle)));
		*rtk_plane->ringheader_ptrs.p_size = htonl(64*1024);
		*rtk_plane->ringheader_ptrs.p_buffer_id = htonl(1);
	}
	*rtk_plane->ringheader_ptrs.p_write_ptr = *rtk_plane->ringheader_ptrs.p_begin_addr;
	*rtk_plane->ringheader_ptrs.p_read_ptr = *rtk_plane->ringheader_ptrs.p_begin_addr;
		   ...
		   enum VO_VIDEO_PLANE layer_nr, struct rtk_rpc_info *rpc_info)
{
	...
	err = rtk_plane_get_cache_config(rtk_plane->rpc_info, &cache_cfg);
	if (err) {
		DRM_ERROR("failed to get cache config\n");
		return err;
	}

	if (cache_cfg == ENUM_MEMORY_CFG_ALL_CACHE)
		rtk_plane->fw_mem_cfg = 1;
	else if (cache_cfg == ENUM_MEMORY_CFG_WITH_UNCACHE)
		rtk_plane->fw_mem_cfg = 0;
	else {
		rtk_plane->fw_mem_cfg = 0;
		DRM_ERROR("UNKNOW FW memory config %d\n", cache_cfg);
	}

	DRM_INFO("%s's fw_mem_cfg = %d\n",
		plane_names[rtk_plane->layer_nr], rtk_plane->fw_mem_cfg);
...
int rtk_plane_get_mixer_by_videoplane(struct rtk_drm_crtc *rtk_crtc, enum VO_VIDEO_PLANE layer_nr)
{
	...
	DRM_INFO("rtk_plane: get_mixer_by_videoplane\n");
...
int rtk_plane_get_mixer_from_cluster(struct rtk_drm_crtc *rtk_crtc, enum VO_VIDEO_PLANE layer_nr)
{
	...
	DRM_INFO("rtk_plane: get_mixer_from_cluster\n");
