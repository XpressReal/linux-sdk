...
struct rtk_drm_vowb {
	...
	u32 instance;
	struct tag_refclock_ptrs shm_refclock;
...
static int rtk_drm_alloc_refclock(struct rtk_drm_vowb *vowb, bool is_cacheable)
{
	...
	rtk_drm_tag_refclock_ptrs_init(&vowb->shm_refclock, r->virt, is_cacheable);
	*vowb->shm_refclock.p_rcd = cpu_to_rpmsg64(rpdev, -1LL);
	*vowb->shm_refclock.p_rcd_ext = cpu_to_rpmsg32(rpdev, -1L);
	*vowb->shm_refclock.p_master_gpts = cpu_to_rpmsg64(rpdev, -1LL);
	*vowb->shm_refclock.p_gpts_timeout = cpu_to_rpmsg64(rpdev, 0LL);
	*vowb->shm_refclock.p_video_system_pts = cpu_to_rpmsg64(rpdev, -1LL);
	*vowb->shm_refclock.p_audio_system_pts = cpu_to_rpmsg64(rpdev, -1LL);
	*vowb->shm_refclock.p_video_rpts = cpu_to_rpmsg64(rpdev, -1LL);
	*vowb->shm_refclock.p_audio_rpts = cpu_to_rpmsg64(rpdev, -1LL);
	*vowb->shm_refclock.p_video_context = cpu_to_rpmsg32(rpdev, -1);
	*vowb->shm_refclock.p_audio_context = cpu_to_rpmsg32(rpdev, -1);
	*vowb->shm_refclock.p_video_end_of_segment = cpu_to_rpmsg32(rpdev, -1);
	*vowb->shm_refclock.p_video_free_run_threshold = cpu_to_rpmsg32(rpdev, 0x7FFFFFFF);
	*vowb->shm_refclock.p_audio_free_run_threshold = cpu_to_rpmsg32(rpdev, 0x7FFFFFFF);
	*vowb->shm_refclock.p_vo_underflow = cpu_to_rpmsg32(rpdev, 0);
	*vowb->shm_refclock.p_ao_underflow = cpu_to_rpmsg32(rpdev, 0);
	vowb->shm_refclock.p_mastership->systemMode = (unsigned char)AVSYNC_FORCED_SLAVE;
	vowb->shm_refclock.p_mastership->videoMode = (unsigned char)AVSYNC_FORCED_MASTER;
	vowb->shm_refclock.p_mastership->audioMode = (unsigned char)AVSYNC_FORCED_MASTER;
	vowb->shm_refclock.p_mastership->masterState = (unsigned char)AUTOMASTER_NOT_MASTER;
	return 0;
}
...
static int rtk_vowb_bind(struct device *dev, struct device *master, void *data)
{
	...
	ret = rtk_drm_ringbuffer_alloc(vowb->dev, get_rpdev(vowb->rpc_info), &vowb->tx,
				       RTK_DRM_VOWB_BRINGBUFFER_SIZE, vowb->rpc_info->is_cacheable);
	if (ret) {
	...
	ret = rtk_drm_alloc_refclock(vowb, vowb->rpc_info->is_cacheable);
	if (ret) {
...
static void rtk_vowb_remove(struct platform_device *pdev)
{
