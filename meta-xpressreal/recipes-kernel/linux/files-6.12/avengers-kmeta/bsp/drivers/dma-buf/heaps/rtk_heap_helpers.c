...
static void *dma_heap_map_kernel(struct heap_helper_buffer *buffer)
{
	...
	if (buffer->heap_buffer.flags & RTK_FLAG_PROTECTED_MASK) {

		if (protect_stub_mode) {
			pr_debug("dma_heap: stub mode active,"
			" allowing map of protected buffer (for debug only)\n");
		} else {
			pr_debug("dma_heap: kernel map denied"
				" for protected buffer\n");
			 ...
			 struct vm_area_struct *vma)
{
	...
	if (heap_buffer->flags & RTK_FLAG_PROTECTED_MASK) {
		if (protect_stub_mode) {
			pr_debug("dma_heap: stub mode active,"
			" allowing mmap of protected buffer (for debug only)\n");
		} else {
			pr_debug("dma_heap: mmap denied for protected buffer\n");
			return -EPERM;
		}
	}
