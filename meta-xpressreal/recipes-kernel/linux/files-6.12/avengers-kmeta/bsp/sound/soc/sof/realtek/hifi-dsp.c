...
static void rtk_hifi_remove(struct snd_sof_dev *sdev)
{
	struct rtk_hifi_priv *priv = sdev->pdata->hw_pdata;
...
__attribute__((unused)) static void set_hifi_pll_and_ssc_control(uint32_t freq_setting)
{
...
static struct snd_soc_acpi_mach *rtk_machine_select(struct snd_sof_dev *sdev)
{
...
static snd_pcm_uframes_t rtk_hifi_pcm_pointer(struct snd_sof_dev *sdev,
				       struct snd_pcm_substream *substream)
{
...
static struct sof_dev_desc snd_sof_rtd1920s_desc = {
	...
	.default_fw_path = {
		[SOF_IPC_TYPE_3] = "rtd1920s",
	},
	.default_tplg_path = {
		[SOF_IPC_TYPE_3] = "rtd1920s",
	},
	.default_fw_filename = {
		[SOF_IPC_TYPE_3] = "sof-adsp-rtd1920s.ri",
	},
...
__attribute__((unused)) static int sof_rtk_probe(struct platform_device *pdev)
{
	...
	sof_pdata->fw_filename = desc->default_fw_filename[0];
	...
	if (fw_path)
		...
		sof_pdata->fw_filename_prefix =
			sof_pdata->desc->default_fw_path[0];

	if (tplg_path)
		sof_pdata->tplg_filename_prefix = tplg_path;
	else
		sof_pdata->tplg_filename_prefix =
			sof_pdata->desc->default_tplg_path[0];
...
__attribute__((unused)) static void sof_rtk_remove(struct platform_device *pdev)
{
